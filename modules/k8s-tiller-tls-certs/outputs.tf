output "ca_tls_certificate_key_pair_secret_namespace" {
  ...
  value       = "${element(concat(kubernetes_secret.ca_secret.*.metadata.0.namespace, list("")), 0)}"
}
...
output "ca_tls_certificate_key_pair_secret_name" {
  ...
  value       = "${element(concat(kubernetes_secret.ca_secret.*.metadata.0.name, list("")), 0)}"
}
...
output "signed_tls_certificate_key_pair_secret_namespace" {
  ...
  value       = "${element(concat(kubernetes_secret.signed_tls.*.metadata.0.namespace, list("")), 0)}"
}
...
output "signed_tls_certificate_key_pair_secret_name" {
  ...
  value       = "${element(concat(kubernetes_secret.signed_tls.*.metadata.0.name, list("")), 0)}"
}
